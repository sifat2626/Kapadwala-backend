...
const createPayment: RequestHandler = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        'An internal server error occurred while creating the subscription.',
    })
  }
}
...
const cancelSubscription: RequestHandler = catchAsync(async (req, res) => {
  ...
  try {
    ...
    return sendResponse(res, {
      ...
      success: true,
      message:
        'Subscription canceled successfully. Access will remain until the end of the billing period.',
      data: {
